...
Nterm_mito_half_life <- mitochondrial_complex |> 
  left_join(HEK_Nterm_Kd_half_life_LaminB_Tcomplex, by = join_by('TERM' == 'UniProt_Accession'), relationship = 'many-to-many')
...
Nterm_mito_half_life |> 
  filter(
    str_detect(complex_name, 'MICOS|ribosom|TIM23|Respiratory|TIM22|TOM')
  ) |> 
  mutate(
    complex_name = case_when(
      ...
      str_detect(complex_name, 'TIM22') ~ 'TIM22 complex related',
      str_detect(complex_name, 'TOM') ~ 'TOM complex related'
    )
  ) |> 
