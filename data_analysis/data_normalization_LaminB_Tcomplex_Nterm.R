...
# curve fitting
HEK_Nterm_curve_fitting_LaminB_Tcomplex <- HEK_Nterm_deg_ratio |> 
  filter(
...
# filter
HEK_Nterm_LaminB_Tcomplex_result <- HEK_Nterm_curve_fitting_LaminB_Tcomplex |> 
  filter(RSS < 0.05) |> 
...
write_csv(HEK_Nterm_LaminB_Tcomplex_result, file = 'data_source/LaminB_Tcomplex_normalization/HEK_Nterm_LaminB_Tcomplex_result.csv')

## half life calculation for Lamin B and T-complex proteoforms
HEK_Nterm_LaminB_Tcomplex_half_life <- HEK_Nterm_LaminB_Tcomplex_result |> 
...
write_csv(HEK_Nterm_LaminB_Tcomplex_half_life, file = 'data_source/LaminB_Tcomplex_normalization/HEK_Nterm_LaminB_Tcomplex.csv')

# ratio calculation for each time point
timepoint <- c(0, 3, 6, 9, 12, 24)
HEK_Nterm_LaminB_Tcomplex_adjusted_ratio <- HEK_Nterm_LaminB_Tcomplex_half_life |> 
  select(Index, UniProt_Accession, Protein.Start, Gene, Entry.Name, lnA, Kd, Kd_adj) |>
  crossing(timepoint) |>
  mutate(
...
write_csv(
  HEK_Nterm_LaminB_Tcomplex_adjusted_ratio, 
  file = 'data_source/LaminB_Tcomplex_normalization/HEK_Nterm_LaminB_Tcomplex_adjusted_ratio.csv'
)

# ratio normalization factor
ratio_norm_facs_Nterm <- HEK_Nterm_LaminB_Tcomplex_adjusted_ratio |> 
  group_by(timepoint) |> 
  summarize(
    ratio_norm_fac = max(ratio)
  )

write_csv(ratio_norm_facs_Nterm, file = 'data_source/LaminB_Tcomplex_normalization/ratio_norm_facs_Nterm.csv')
